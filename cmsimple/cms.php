...
CMSimple is available under three different licenses:
...
2) Linkware / CMSimple Link Requirement Licence
...
To purchase a commercial license, please go to http://www.cmsimple.com/?Purchase_Licence
...
For changelog, downloads and information about installation, please see http://www.cmsimple.com
...
#	$c = explode('§', preg_replace("/(<h[1-".$cf['menu']['levels']."][^>]*>)/i", "§\\1", str_replace('Ð§', 'Ð§', rf($pth['file']['content']))));

#	$c = explode('§', preg_replace("/(<h[1-".$cf['menu']['levels']."][^>]*>)/i", "§\\1", rf($pth['file']['content'])));
...
function head() {
	...
	return $t.tag('meta name="generator" content="CMSimple 3.2"').tag('link rel="stylesheet" href="'.$pth['file']['stylesheet'].'" type="text/css"').$hjs;
}
